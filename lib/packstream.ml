...
let rec serialize message =
  ...
  | Node {id; labels; properties} ->
      let%bitstring header = {| 0xb34e : 16 |} in
      Bitstring.concat
        [header; serialize_int id; serialize_list (List.map ~f:(fun s -> String s) labels); serialize_dict properties]
  | Relationship {id; start_node_id; end_node_id; typ; properties} ->
      let%bitstring header = {| 0xb552 : 16 |} in
      Bitstring.concat
        [ header;
          serialize_int id;
  ...
  | UnboundRelationship {id; typ; properties} ->
      let%bitstring header = {| 0xb372 : 16 |} in
      Bitstring.concat [header; serialize_int id; serialize_string typ; serialize_dict properties]
  | Path {nodes; rels; ids} ->
      let%bitstring header = {| 0xb350 : 16 |} in
      Bitstring.concat
        [ header;
  ...
  | LocalDateTime {seconds; nanoseconds} ->
      let%bitstring header = {| 0xb264 : 16 |} in
      Bitstring.concat [header; serialize_int seconds; serialize_int nanoseconds]
  | Duration {months; days; seconds; nanoseconds} ->
      let%bitstring header = {| 0xb445 : 16 |} in
      Bitstring.concat
        [header; serialize_int months; serialize_int days; serialize_int seconds; serialize_int nanoseconds]
  | Point2D {srid; x; y} ->
      let%bitstring header = {| 0xb358 : 16 |} in
      Bitstring.concat [header; serialize_int srid; serialize_float x; serialize_float y]
  | Point3D {srid; x; y; z} ->
      let%bitstring header = {| 0xb459 : 16 |} in
...
and serialize_list lst =
  ...
  | l when l > 65535 ->
      let%bitstring header = {| 0xD6 : 8; (Int32.of_int_exn l) : 32|} in
      Bitstring.concat (header :: elems)
  | l when l > 255 ->
      let%bitstring header = {| 0xD5 : 8; l : 16 : unsigned|} in
      Bitstring.concat (header :: elems)
  | l when l > 15 ->
      let%bitstring header = {| 0xD4 : 8; l : 8 : unsigned|} in
      Bitstring.concat (header :: elems)
  | l ->
      let%bitstring header = {| 0x9 : 4; l : 4|} in
      Bitstring.concat (header :: elems)
...
and serialize_dict dct =
  ...
  | l when l > 65535 ->
      let%bitstring header = {| 0xDA : 8; (Int32.of_int_exn l) : 32|} in
      Bitstring.concat (header :: elems)
  | l when l > 255 ->
      let%bitstring header = {| 0xD9 : 8; l : 16 : unsigned|} in
      Bitstring.concat (header :: elems)
  | l when l > 15 ->
      let%bitstring header = {| 0xD8 : 8; l : 8 : unsigned|} in
      Bitstring.concat (header :: elems)
  | l ->
      let%bitstring header = {| 0xA : 4; l : 4|} in
      Bitstring.concat (header :: elems)
