...
let run_test_cases cases =
  let internal (expected, value) =
    let bs = Bitstring.bitstring_of_string expected in
    assert_equal bs (serialize value)
  in
  List.iter ~f:internal cases

let test_null _ctx = run_test_cases ["\xc0", Null]
let test_false _ctx = run_test_cases ["\xc2", False]
let test_true _ctx = run_test_cases ["\xc3", True]
...
let test_tiny_int _ctx =
  let neg_cases =
    [ "\xf0", Int (-16L);
      ...
      "\xff", Int (-1L) ]
  and pos_cases =
    List.map (List.init 128 ~f:Fun.id) ~f:(fun n -> String.of_char @@ Char.of_int_exn n, Int (Int64.of_int_exn n))
  in
...
let test_8_byte_int _ctx =
  ...
  let cases =
    List.map numbers ~f:(fun i -> "\xc8" ^ String.of_char @@ Char.of_int_exn @@ (256 + i), Int (Int64.of_int i))
  in
  run_test_cases cases

let test_16_byte_int _ctx =
  let cases =
    ["\xC9\x80\x00", Int (-32768L); "\xC9\xff\x7f", Int (-129L); "\xC9\x00\x80", Int 128L; "\xC9\x7f\xff", Int 32767L]
  in
  run_test_cases cases
...
let test_float _ctx = run_test_cases ["\xC1\x3F\xF3\xAE\x14\x7A\xE1\x47\xAE", Float 1.23]
...
let test_structs _ctx =
  let cases =
    [ "\xB0\x02", Struct {length = 0; tag = 2; fields = []};
      ...
      "\xB4\x02\x00\x80\x90\x00", Struct {length = 4; tag = 2; fields = [Int 0L; String ""; List []; Int 0L]};
      ...
      ( "\xB8\x02\x00\x80\x90\x00\x80\x90\x00\x80",
        Struct
          {length = 8; tag = 2; fields = [Int 0L; String ""; List []; Int 0L; String ""; List []; Int 0L; String ""]} );
      ( "\xB9\x02\x00\x80\x90\x00\x80\x90\x00\x80\x90",
        Struct
          { length = 9;
            tag = 2;
            fields = [Int 0L; String ""; List []; Int 0L; String ""; List []; Int 0L; String ""; List []] } );
      ( "\xBA\x02\x00\x80\x90\x00\x80\x90\x00\x80\x90\x00",
        Struct
          { length = 10;
            tag = 2;
            fields = [Int 0L; String ""; List []; Int 0L; String ""; List []; Int 0L; String ""; List []; Int 0L] } );
      ...
      ( "\xB5\x52\x0B\x02\x03\x85KNOWS\xA1\x84name\x87example",
        Relationship
          {id = 11L; start_node_id = 2L; end_node_id = 3L; typ = "KNOWS"; properties = ["name", String "example"]} );
